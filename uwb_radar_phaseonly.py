    ...
    def preprocessing(self):
        ...
        while not self.end:
            ...
            # with self.read_condition:
                ...
                if self.phase is not None and self.phase.shape[0] > 12*40:
                    if ycl == 1:
                        print('init')
                    self.phase = self.phase[-(11*40):, :]
                    rawdata = self.phase[-(11*40):, :-1].copy()
        ...
        while not self.end:
            ...
            # with self.read_condition:
                ...
                # self.leiji = save.copy()
                    self.phase = save.copy()
                    # print(leiji)
                else:
                # self.leiji = np.vstack((self.leiji, save))
                    self.phase = np.vstack((self.phase, save))
    ...
    def data_storage_raw(self):
        ...
        while not self.end:
            ...
            if self.leiji_data_storage.shape[0] >= 40 * 60 * 1:
                sio.savemat('D:/AllAboutPostGraduateWork/项目/301医院/二阶段/MCI_monitor/data_storage/20220516indoorwyl{num}.mat'.format(num=num), {'data': self.leiji_data_storage})
